...
void BarrierSyncServer::synchronize(core_id_t core_id, SubsecondTime time)
{
   ...
   // printf("Core %d entry (master core %d, thread %d, ffwd %d)\n", core_id, master_core_id, thread_me, m_fastforward);
   // printf("Received 'SIM_BARRIER_WAIT' from Core(%i), Time(%s)\n", core_id, itostr(time).c_str());
...
void BarrierSyncServer::threadExit(HooksManager::ThreadTime *argument)
{
...
void BarrierSyncServer::releaseThread(thread_id_t thread_id)
{
   for (core_id_t core_id = 0; core_id < (core_id_t)Sim()->getConfig()->getApplicationCores(); core_id++)
   {
...
bool BarrierSyncServer::isCoreRunning(core_id_t core_id, bool siblings)
{
   Core *core = Sim()->getCoreManager()->getCoreFromID(core_id);
...
void BarrierSyncServer::advance()
{
...
bool BarrierSyncServer::barrierRelease(thread_id_t caller_id, bool continue_until_release)
{
...
void BarrierSyncServer::doRelease(int n)
{
...
void BarrierSyncServer::setDisable(bool disable)
{
...
void BarrierSyncServer::setGroup(core_id_t core_id, core_id_t master_core_id)
{
...
void BarrierSyncServer::setFastForward(bool fastforward, SubsecondTime next_barrier_time)
{
...
void BarrierSyncServer::printState(void)
{
