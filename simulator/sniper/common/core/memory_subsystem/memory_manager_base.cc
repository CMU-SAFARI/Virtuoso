...
#include "fast_detailed/memory_manager.h"
#include "log.h"
#include "config.hpp"
      ...
      Core* core, Network* network, ShmemPerfModel* shmem_perf_model)
{
   ...
   switch (caching_protocol)
   {
      ...
      case FAST_DETAILED:
         return new FastDetailed::MemoryManager(core, network, shmem_perf_model);
...
MemoryManagerBase::parseProtocolType(String& protocol_type)
{
   if (protocol_type == "parametric_dram_directory_msi")
      ...
      return FAST_NEHALEM;
   else if (protocol_type == "fast_detailed")
      return FAST_DETAILED;
   else
