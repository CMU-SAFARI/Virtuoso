...
UInt64 MagicServer::Magic_unlocked(thread_id_t thread_id, core_id_t core_id, UInt64 cmd, UInt64 arg0, UInt64 arg1)
{
   switch(cmd)
   {
      ...
      case SIM_CMD_MALLOC:
      {
            std::cout<<"[kole kole] Hey, we are in malloc with core "<<core_id<<std::endl;
      }
      case SIM_CMD_VIRTU_PIPE:
      {
            std::cout<<"[Virtuoso] We received the pipe from VirtuOS"<< std::endl;
            char str[256];
            ...
            std::cout << "[Virtuoso] The pipe is: " << str << std::endl;

            std::string pipeName(str);
            std::ofstream pipe(pipeName, std::ios::binary); // Open the pipe in binary mode

            if (pipe.is_open()) {
               std::string line = "Hello VirtuOS from Virtuoso!";
               pipe << line << std::endl;
            } else {
               std::cout << "[Virtuoso] Unable to open the pipe" << std::endl;
            }
