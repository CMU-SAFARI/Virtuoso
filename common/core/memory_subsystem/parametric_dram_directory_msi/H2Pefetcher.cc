...
namespace ParametricDramDirectoryMSI
{
	...
	std::vector<query_entry> H2Prefetcher::performPrefetch(IntPtr address, IntPtr eip, Core::lock_signal_t lock, bool modeled, bool count, PageTable *pt)
	{
		...
		{
			result.push_back(PTWTransparent(address + (C_address - B_address), eip, lock, modeled, count, pt));
			result.push_back(PTWTransparent(address + (B_address - A_address), eip, lock, modeled, count, pt));
		}
