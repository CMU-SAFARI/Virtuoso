...
namespace ParametricDramDirectoryMSI
{
...

	{
		...
		try
		{
			...
			m_mmu = MMUFactory::createMemoryManagementUnit(mmu_type, core, this, shmem_perf_model, NULL);
		...
		Core::MemModeled modeled)
	{
		...
		pair<SubsecondTime, IntPtr> translation_result;
		if (mmu_type == "midgard")
		{
		...
		IntPtr physical_address = translation_result.second & ~(getCacheBlockSize() - 1);
		HitWhere::where_t result = m_cache_cntlrs[mem_component]->processMemOpFromCore(
			...
			modeled == Core::MEM_MODELED_NONE || modeled == Core::MEM_MODELED_COUNT ? false : true,
			modeled == Core::MEM_MODELED_NONE ? false : true, CacheBlockInfo::block_type_t::NON_PAGE_TABLE, translation_result.first);
		...
		if (mmu_type == "midgard")
		{
			...
			if (result == HitWhere::where_t::DRAM || result == HitWhere::where_t::DRAM_CACHE || result == HitWhere::where_t::DRAM_LOCAL || result == HitWhere::where_t::DRAM_REMOTE)
			{
				...
				incrElapsedTime(translation_result.first, ShmemPerfModel::_USER_THREAD);
			}
		}
		return result;
	}
