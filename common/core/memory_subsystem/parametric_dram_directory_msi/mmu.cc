...
namespace ParametricDramDirectoryMSI
{
	...
	MemoryManagementUnit::MemoryManagementUnit(Core *_core, MemoryManager *_memory_manager, ShmemPerfModel *_shmem_perf_model, MemoryManagementUnitBase *_host_mmu) : MemoryManagementUnitBase(_core, _memory_manager, _shmem_perf_model, _host_mmu), memory_manager(_memory_manager)
	{
		instantiateMetadataTable();
	...
	{
		String metadata_table_name = Sim()->getCfg()->getString("perf_model/mmu/metadata_table_name");
	...
	pair<SubsecondTime, IntPtr> MemoryManagementUnit::performAddressTranslation(IntPtr eip, IntPtr address, bool instruction, Core::lock_signal_t lock, bool modeled, bool count)
	{
		...
		{
			MetadataWalkResult mwresult = metadata_table->initializeWalk(address);
			vector<IntPtr> maccesses = get<1>(mwresult);
			metadata_table_walk_latency = get<0>(mwresult);
			for (int i = 0; i < maccesses.size(); i++)
			{
				...
				packet.count = count;
				packet.type = get<2>(mwresult);

				SubsecondTime latency = accessCache(packet);
				metadata_table_walk_latency += latency;
			}
		}
