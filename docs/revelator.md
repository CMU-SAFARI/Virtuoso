...
browser. The reviewer-facing reproduction flow (build, traces, suites, figures) is
not on `main` — it ships with the artifact on the `revelator-artifact-release`
branch, at [`experiments/ae/`](https://github.com/CMU-SAFARI/Virtuoso/tree/revelator-artifact-release/experiments/ae).
...
The general-purpose `experiments/clist.yaml` carries standalone Revelator entries
(`revelator`, `revelator-3hash-utilization-sweep`). The paper's single-core suites
(`revelator_headtohead`, `revelator_thp_headtohead`, `revelator_util_sweep`) live in
`experiments/clist_revelator.yaml` on the artifact branch, not on `main`.

### Artifact evaluation

`main` carries the implementation, not the reproduction harness. The four suites —
`revelator`, `revelator_thp`, `utilsweep`, `multicore` — and the one-command driver
that runs them are on the `revelator-artifact-release` branch:
...
native VA→PA mapping microbenchmarks are **not** in this repository. Area and
static-power numbers cannot be reproduced from it.
