...
# claim -> (generator, suite(s), exp-dir, parser, parser-extra-args)
ae_claim_cfg() {  # sets GEN SUITE DIR PARSER PARGS ; returns 1 on unknown claim
  case "$1" in
    head8mb)   GEN=sc; SUITE="trail_comparison_v4";             DIR="ae_head8mb"; PARSER=parse_headtohead.py; PARGS="--suffix ";;
    head2mb)   GEN=sc; SUITE="trail_comparison_v4_nuca2mb";     DIR="ae_head2mb"; PARSER=parse_headtohead.py; PARGS="--suffix -nuca2mb";;
    table5)    GEN=sc; SUITE="trail-pte-budget-grid-corrected"; DIR="ae_table5";  PARSER=parse_table5.py;    PARGS="";;
    table6)    GEN=sc; SUITE="sidecar-payload-sweep-corrected"; DIR="ae_table6";  PARSER=parse_table6.py;    PARGS="";;
    pqsweep)   GEN=sc; SUITE="pq-size-sweep";                   DIR="ae_pqsweep"; PARSER=parse_pqsweep.py;   PARGS="";;
    multicore) GEN=mc; SUITE="prefetcher_v4_diverse_4core prefetcher_v4_diverse_4core_x60"; DIR="ae_multicore"; PARSER=parse_multicore.py; PARGS="";;
    *) echo "unknown claim: $1 (valid: head8mb head2mb table5 table6 pqsweep multicore)" >&2; return 1;;
  esac
