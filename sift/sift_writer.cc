...
Sift::Writer::Writer(const char *filename, GetCodeFunc getCodeFunc, bool useCompression, const char *response_filename, uint32_t id, bool arch32, bool requires_icache_per_insn, bool send_va2pa_mapping, GetCodeFunc2 getCodeFunc2, void* getCodeFunc2Data, bool revise)
   : response(NULL)
...
void Sift::Writer::MallocEnter(uint64_t pointer, uint64_t size)
{
   if (!output)
   {
      return;
   ...
   Record rec;
   rec.Other.zero = 0;
   rec.Other.type = RecOtherMallocEnter;
  
   //std::cout << "[SIFT_WRITER] MALLOC ENTER: " << size << std::endl;
   rec.Other.size = 2 * sizeof(uint64_t);
   output->write(reinterpret_cast<char*>(&rec), sizeof(rec.Other));
  
   
   output->write(reinterpret_cast<char*>(&pointer), sizeof(uint64_t));
   output->write(reinterpret_cast<char*>(&size), sizeof(uint64_t));
}

void Sift::Writer::FreeEnter(uint64_t pointer)
{
   if (!output)
   {
      return;
   ...
   Record rec;
   rec.Other.zero = 0;
   rec.Other.type = RecOtherFreeEnter;
  
   //std::cout << "[SIFT_WRITER] FREE ENTER: " << size << std::endl;
   rec.Other.size = 1 * sizeof(uint64_t);
   output->write(reinterpret_cast<char*>(&rec), sizeof(rec.Other));
...
void Sift::Writer::CallocEnter(uint64_t pointer, uint64_t size)
{
   if (!output)
   {
      return;
   ...
   Record rec;
   rec.Other.zero = 0;
   rec.Other.type = RecOtherCallocEnter;
  
   //std::cout << "[SIFT_WRITER] CALLOC ENTER: " << size << std::endl;
   rec.Other.size = 2 * sizeof(uint64_t);
   output->write(reinterpret_cast<char*>(&rec), sizeof(rec.Other));
...
void Sift::Writer::ReallocEnter(uint64_t init_pointer, uint64_t pointer, uint64_t size)
{
   if (!output)
   {
      return;
   ...
   Record rec;
   rec.Other.zero = 0;
   rec.Other.type = RecOtherReallocEnter;
  
   //std::cout << "[SIFT_WRITER] REALLOC ENTER: " << size << std::endl;
   rec.Other.size = 3 * sizeof(uint64_t);
   output->write(reinterpret_cast<char*>(&rec), sizeof(rec.Other));
  
   
   output->write(reinterpret_cast<char*>(&init_pointer), sizeof(uint64_t));
   output->write(reinterpret_cast<char*>(&pointer), sizeof(uint64_t));
   output->write(reinterpret_cast<char*>(&size), sizeof(uint64_t));
}

void Sift::Writer::Execv()
{
   if (!output)
   {
      return;
   ...
   Record rec;
   rec.Other.zero = 0;
   rec.Other.type = RecOtherExecv;
  
   std::cout << "[SIFT_WRITER] Execv " << std::endl;
   rec.Other.size = 0 * sizeof(uint64_t);
   output->write(reinterpret_cast<char*>(&rec), sizeof(rec.Other));
   output->flush();
