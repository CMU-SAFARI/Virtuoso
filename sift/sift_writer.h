...
namespace Sift
{
   class Writer
   {
      ...
      public:
         Writer(const char *filename, GetCodeFunc getCodeFunc, bool useCompression = false, const char *response_filename = "", uint32_t id = 0, bool arch32 = false, bool requires_icache_per_insn = false, bool send_va2pa_mapping = false, GetCodeFunc2 getCodeFunc2 = NULL, void *GetCodeFunc2Data = NULL, bool revise = false);
         ~Writer();
         void End();
         ...
         bool IsOpen();
         void Execv();
	 void MallocEnter(uint64_t pointer, uint64_t size);
         void CallocEnter(uint64_t pointer, uint64_t size);
	 void ReallocEnter(uint64_t init_pointer, uint64_t pointer, uint64_t size);
	 void FreeEnter(uint64_t pointer);
         void setRevise() { revise = true;}
