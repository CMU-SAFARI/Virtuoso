...
{
   std::cout << "Opening output for thread: " << threadid <<  " "<<  KnobOutputFile.Value() <<std::endl;
   ...
   if (res < 0) {
      if (errno == ENOENT) {
         ...
         std::cerr << "Error: " << filename << " fail" << std::endl;
      }
   }
   else{
   ...
   {
      std::cout << "Writer is already open for thread: " << threadid << std::endl;
...
void reviseFile(THREADID threadid){

   std::cout << "A child process is revising the file" << std::endl;
   if (KnobVerbose.Value())
   {
      std::cerr << "[SIFT_RECORDER:" << app_id << ":" << thread_data[threadid].thread_num << "] Recorded " << thread_data[threadid].icount_detailed;
      if (thread_data[threadid].icount > thread_data[threadid].icount_detailed)
