...
VOID sendInstruction(THREADID threadid, ADDRINT addr, UINT32 size, UINT32 num_addresses, BOOL is_branch, BOOL taken, BOOL is_predicate, BOOL executing, BOOL isbefore, BOOL ispause)
{
   ...
   if (thread_data[threadid].icount_detailed % 10000 == 0)
   {
      ...
      String vma_acro = ".vma";
      String vma_file = KnobOutputFile.Value() + vma_acro;
      ...
      outFile.open(vma_file.c_str(), std::ios::app);
      ...
      outFile << "VMA: " << thread_data[threadid].icount_detailed << " " << std::endl;
      std::string line;
      while (std::getline(inFile, line))
      {
         ...
         if (hyphenPos != std::string::npos)
         {
            ...
            outFile << line.substr(0, spacePos) << std::endl;
         }
      }
...
VOID MallocInsert(CHAR *name, ADDRINT size)
{
...
void mallocImgFunction(IMG img, void *args)
{
   RTN mallocRtn = RTN_FindByName(img, MALLOC);
   if (RTN_Valid(mallocRtn))
   {
      RTN_Open(mallocRtn);
      RTN_InsertCall(mallocRtn, IPOINT_BEFORE, (AFUNPTR)MallocInsert, IARG_ADDRINT,
                     MALLOC, IARG_FUNCARG_ENTRYPOINT_VALUE, 0, IARG_END);
      RTN_Close(mallocRtn);
   }
}
...
void initRecorderBase()
{
   ...
   // IMG_AddInstrumentFunction(mallocImgFunction, 0);
}
