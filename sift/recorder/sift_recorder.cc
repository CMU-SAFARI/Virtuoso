...
BOOL followChild(CHILD_PROCESS childProcess, VOID *val)
{
   if (any_thread_in_detail)
   {
      ...
      fprintf(stderr, "EXECV allowed\n");
      INT appArgc;
      CHAR const * const * appArgv;
      OS_PROCESS_ID pid = CHILD_PROCESS_GetId(childProcess);
      std::cout << "pid of child (execv): " << pid << std::endl;
      CHILD_PROCESS_GetCommandLine(childProcess, &appArgc, &appArgv);
      //concat all char* of argv_pin into one string
      std::string argv_pin_full = "";
      for (int i = 0; i < (argc_pin-3); i++){
         argv_pin_full += argv_pin[i];
         argv_pin_full += " ";
      }
      std::cout <<  "appArgc: " << appArgc << std::endl;
      std::cout <<  "appArgv: " << argv_pin_full << std::endl;

       INT pinArgc = argc_pin;
       CHAR **pinArgv;
       pinArgv = argv_pin;
...
int main(int argc, char **argv)
{
   ...
   //std::cout<<fast_forward_target<<" "<<KnobUseROI.Value()<<" "<<KnobMPIImplicitROI.Value();
