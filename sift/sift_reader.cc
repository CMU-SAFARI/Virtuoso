...
bool Sift::Reader::Read(Instruction &inst)
{
   ...
   while(!m_seen_end)
   {
      ...
      if (byte == 0)
      {
	 //std::cerr << "Hello OTHER REQUEST" << std::endl;
	 // Other
	    ...
	    case RecOtherMallocEnter:
	    {
	       
 	       assert(rec.Other.size == 2 * sizeof(uint64_t));
               uint64_t pointer;
	       uint64_t size;
               input->read(reinterpret_cast<char*>(&pointer), sizeof(uint64_t));
               input->read(reinterpret_cast<char*>(&size), sizeof(uint64_t));

	       if(handleMallocEnterFunc)
		       handleMallocEnterFunc(handleMallocEnterArg,pointer,size);

              
	       //std::cout << "[SIFT_READER] MallocEnter -  " << " Pointer: " << pointer << " Size: " << size << std::endl;
	       break;
	    }
	    case RecOtherCallocEnter:
	    {
	       assert(rec.Other.size == 2 * sizeof(uint64_t));
               uint64_t pointer;
	       uint64_t size;
	       ...
	       if(handleCallocEnterFunc)
		       handleCallocEnterFunc(handleCallocEnterArg,pointer,size);

	       //std::cerr << "[SIFT_READER] CallocEnter -  " << " Pointer: " << pointer << " Size: " << size << std::endl;
               
	       break;

	    }
	    case RecOtherReallocEnter:
	    {
	       assert(rec.Other.size == 3 * sizeof(uint64_t));
               
	       uint64_t pointer;
	       uint64_t size;
	       uint64_t init_pointer;

               input->read(reinterpret_cast<char*>(&init_pointer), sizeof(uint64_t));
	       input->read(reinterpret_cast<char*>(&pointer), sizeof(uint64_t));
               input->read(reinterpret_cast<char*>(&size), sizeof(uint64_t));
	       if(handleReallocEnterFunc)	
		       handleReallocEnterFunc(handleReallocEnterArg,init_pointer,pointer,size);

	       //std::cerr << "[SIFT_READER] ReallocEnter -  " << " Init pointer: " << init_pointer << " Pointer: " << pointer << " Size: " << size << std::endl;
               
	       break;


	    }
	    case RecOtherFreeEnter:
	    {
	       assert(rec.Other.size == 1 * sizeof(uint64_t));
               
	       uint64_t pointer;

               input->read(reinterpret_cast<char*>(&pointer), sizeof(uint64_t));
	       if(handleFreeEnterFunc)
		       handleFreeEnterFunc(handleFreeEnterArg,pointer);
       ...
       case RecOtherExecv:
       {
         //print contents of icache which is an unordered map
         ...
         std::cout << "The writer is dead, long live the writer!" << std::endl;
         break;
      ...
      uint8_t size;
      uint64_t addr;
      //std::cout << "[SIFT_READER] Reading instruction" << std::endl;
      if ((byte & 0xf) != 0)
      {
         // Instruction
...
const Sift::StaticInstruction* Sift::Reader::staticInfoInstruction(uint64_t addr, uint8_t size)
{
   ...
   //std::cout << "Sift::Reader::" << __FUNCTION__ << ": addr = " << addr << ", size = " << size << std::endl;
